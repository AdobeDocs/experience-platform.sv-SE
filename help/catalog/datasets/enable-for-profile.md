...
Du kan aktivera en datauppsättning för kundprofil och identitetstjänst i realtid direkt när den skapas eller när som helst efter att datauppsättningen har skapats. Om du vill aktivera en datamängd som redan har skapats följer du stegen för [konfigurera en befintlig datamängd](#configure-an-existing-dataset) som hittas senare i det här dokumentet.
...
Om du vill skapa en datauppsättning som är aktiverad för profilen kan du använda en POST-förfrågan till `/dataSets`-slutpunkten.
...
Begärandetexten innehåller en `path` till två typer av taggar, `unifiedProfile` och `unifiedIdentity`. `value` för var och en är matriser som innehåller strängen `enabled:true`.
