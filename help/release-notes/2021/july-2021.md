...
description: Versionsinformation juli 2021 för Adobe Experience Platform.
...
| Funktion | Beskrivning |
| ------- | ----------- |
| Kontrollpanel för destinationer | Du kan nu använda kontrollpanelen för övervakning för att övervaka dataflöden för mål. Om du vill veta mer kan du läsa självstudiekursen om [övervaka destinationer i användargränssnittet](../../dataflows/ui/monitor-destinations.md#monitoring-destinations-dashboard) |

Mer allmän information om dataflöden finns i [dataflödesöversikt](../../dataflows/home.md). Mer information om destinationer finns i [destinationer, översikt](../../destinations/home.md).
...
| Funktion | Beskrivning |
| --- | --- |
| Telekommunikationsfilter | När du lägger till fältgrupper i ett schema i användargränssnittet kan du nu filtrera efter telekombranschen. Se [affärsrelationsdiagram (ERD) för telekomindustrin](../../xdm/schema/industries/telecom.md) för att se en rekommenderad datamodell för användning inom telekom. |

Mer allmän information om XDM in Platform finns i [XDM - systemöversikt](../../xdm/home.md).

## Frågetjänst {#query}

Med frågetjänsten kan du använda standard-SQL för att fråga data i Adobe Experience Platform, vilket stöder en mängd olika användningsfall för analys och datahantering. Det är ett serverlöst verktyg som gör att du kan koppla ihop datauppsättningar från datasjön och samla in frågeresultaten som en ny datauppsättning som kan användas för rapportering, datavetenskap eller för att lägga in data i realtidskundprofilen.
...
| Funktion | Beskrivning |
| ------- | ----------- |
| Schemalagda frågor | Du kan nu schemalägga frågor i plattformen med hjälp av Frågeredigeraren. Läs mer i dokumentationen om [Frågeredigeraren](../../query-service/ui/user-guide.md#scheduled-queries). |
...
| [!DNL Salesforce Marketing Cloud] (Beta) | Nu kan du ansluta [!DNL Salesforce Marketing Cloud] till Experience Platform med [!DNL Flow Service] API eller gränssnittet. Se [[!DNL Salesforce Marketing Cloud] anslutningsöversikt](../../sources/connectors/marketing-automation/salesforce-marketing-cloud.md) för mer information. |
