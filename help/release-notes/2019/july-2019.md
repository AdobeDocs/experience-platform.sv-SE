...
| Funktion | Beskrivning |
|---|---|
| Förbättrat ikonsystem | Gränssnittet [!DNL Schema Editor] har uppdaterat ikoner som bättre motsvarar den övergripande [!DNL Platform]-designen. |
| Stöd för JSON-schemaegenskaper och -begränsningar för skalarer | Gränssnittet [!DNL Schema Editor] innehåller nu valfria fält för att definiera begränsningarna [JSON Schema draft-6](https://tools.ietf.org/html/draft-wright-json-schema-01) för skalära fält, till exempel `default` och `required`. |
| Stöd för JSON-schemaegenskaper och -begränsningar för arrayer | Gränssnittet [!DNL Schema Editor] innehåller nu valfria fält för att definiera begränsningarna [JSON Schema draft-6](https://tools.ietf.org/html/draft-wright-json-schema-01) för matrisfält som `minItems`, `maxItems` och `uniqueItems`. |
...
* Ett fel har korrigerats där API:t inte kunde hantera URL:er med plustecknet (`+`).
