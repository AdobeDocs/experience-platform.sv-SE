...
Produktdokumentation finns på [anslutningsöversikt](../../sources/home.md).
...
| [!DNL Advertising Cloud] | DSP loggar från [!DNL Advertising Cloud] är tillgängliga i [!DNL Platform]. |
...
Direktinmatning skickar data till [!DNL Experience Platform] i realtid från klient- och serverbaserade enheter. Du kan uppdatera till [!DNL Real-Time Customer Profiles] med mindre fördröjning med stöd för autentiserad datainsamling och nyttolaster med flera poster.
...
Adobe Experience Platform har stöd för integrering med databehandlingsverktyg som kan extrahera, omvandla och läsa in (ETL) data från många olika källor till [!DNL Experience Platform], säkerställa att data överensstämmer med de korrekta specifikationerna för användning av [!DNL Platform] tjänster. Ett antal partners erbjuder förkonfigurerade anslutningar för [!DNL Experience Platform]. Det finns även dokumentation för utveckling av ETL-anslutningar.
...
XDM är den mekanism som gör att [!DNL Experience Clou]d, som drivs av Adobe Experience Platform, för att leverera rätt budskap till rätt person, i rätt kanal, i precis rätt ögonblick.
...
| Funktion | Beskrivning |
| -----------| ---------- |
| [!DNL Schema Registry] | The [!DNL Schema Registry] har ett användargränssnitt ([!DNL Schema Editor]) och RESTful API från vilket alla schemarelaterade resurser kan visas och hanteras. The [!DNL Schema Editor] och [!DNL Schema Registry] API kan också användas för att skapa och hantera nya scheman och resurser som är unika för varje IMS-organisation. |
| [!DNL Schema Library] | The [!DNL Schema Library] innehåller alla [!DNL Schema Registry] Resurser, inklusive sådana som definieras av en enskild IMS-organisation och sådana som Adobe tillhandahåller för alla användare. [!DNL Experience Platform] partners och leverantörer vars program är integrerade med [!DNL Experience Platform]. |
...
[!DNL Profile] är ett generiskt uppslagsarkiv som sammanfogar data över olika företagsdatatillgångar och ger åtkomst till dessa data i form av konsumentprofiler i realtid och relaterade tidsseriehändelser, som kallas fackvyn. Den enhetliga vyn hjälper till att informera om och förstärka åtgärder över olika kanaler, [!DNL Platform]och integreringar med Adobe. Dessutom [!DNL Profile] används för maskininlärning och Sensei. [!DNL Profile] API:er kan också användas direkt för att berika funktionaliteten hos tredjepartslösningar, CRM-system och företagslösningar.
