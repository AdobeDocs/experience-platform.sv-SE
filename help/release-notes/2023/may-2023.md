...
| Destination SDK-stöd för gruppering av exporterade målgrupper baserat på kopplingsregler. | När du skapar ett filbaserat mål med Destination SDK kan du nu konfigurera grupperingen av exporterade målgrupper i en eller flera filer, baserat på sammanfogningsprincipen. <br><br> Dessutom kan du nu inkludera sammanfogningsprincip-ID:t och sammanfogningsprincipnamnet i de exporterade filnamnen med hjälp av de dedikerade mallmakrona. <br><br>Mer information om hur du använder parametern [ och de nya mallmakrona för filnamn finns i ](../../destinations/destination-sdk/functionality/destination-configuration/batch-configuration.md)dokumentationen för gruppkonfiguration`segmentGroupingEnabled`. |
...
| Fältgrupper | [[!UICONTROL XDM Individual Prospect Profile]](https://github.com/adobe/xdm/pull/1721/files) | Alternativet `partnerProspect` för metadatataggar lades till i klassen [!UICONTROL XDM Individual Prospect Profile]. |
...
| Fältgrupper | [[!UICONTROL MediaAnalytics Interaction Details]](https://github.com/adobe/xdm/pull/1716/files) | Ett nytt fält som är relaterat till medierapportering har lagts till. |
...
| [!DNL Google Ads] autentiseringsuppdateringar | Du kan nu ange ett användar-ID för inloggningskund när du autentiserar ditt [!DNL Google Ads]-källkonto för att hämta rapportdata från en viss operativkund. Läs [[!DNL Google Ads] källdokumentationen](../../sources/connectors/advertising/ads.md) för mer information. |
| [!DNL Google PubSub] autentiseringsuppdateringar | Du kan nu definiera åtkomstbehörigheter för [!DNL Google PubSub]-källan när du skapar ett nytt konto. Använd projektbaserad autentisering för att tillåta åtkomst på rotnivå eller använd ämne- och prenumerationsbaserad autentisering för att begränsa åtkomst till ett visst ämne och en viss prenumerationsström. Läs [[!DNL Google PubSub] källdokumentationen](../../sources/connectors/cloud-storage/google-pubsub.md) för mer information. |
