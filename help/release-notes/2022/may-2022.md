...
Experience Platform har en rad funktioner för datahygien som gör att du kan hantera lagrade data genom att programmatiskt ta bort konsumentposter och datauppsättningar. Med arbetsytan [!UICONTROL Data Hygiene] i användargränssnittet eller genom anrop till API:t för datahygien kan du hantera dina datalager för att se till att informationen används som förväntat, uppdateras när felaktiga data behöver korrigeras och tas bort när organisationsprofiler anser det nödvändigt.
...
| Integrering med dataflöden | Använd källkatalogen i Platform för att komma åt data på Platform Edge Network, inklusive Data Prep för datainsamling och förbättrat stöd för varningar om dataprep. Mer information finns i [Adobe-datakällans översikt](../../sources/connectors/adobe-applications/data-collection.md). |
...
| Stöd för inhämtning av filer med `ISO-8859-1`-kodning | Använd parametern `encoding` för att importera `ISO-8859-1`-kodade filer med en molnlagringskälla till plattformen med API:t [!DNL Flow Service]. Mer information finns i guiden om att [skapa en molnlagringskällanslutning](../../sources/tutorials/api/collect/cloud-storage.md). |
