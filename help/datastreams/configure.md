...
![Alternativ för att redigera eller ta bort ett befintligt dataflöde.](assets/configure/edit-datastream.png)

## Skapa ett datastream {#create}

Om du vill skapa ett datastream börjar du med att välja **[!UICONTROL New Datastream]**.

![Välj ny dataström.](assets/configure/new-datastream-button.png)
...
![Grundläggande konfiguration för ett datastream.](assets/configure/configure.png)
...
Om du aktiverar något av fälten ovan för datainsamling bör du kontrollera att du har angett arrayegenskapen [`context`](/help/collection/js/commands/configure/context.md) korrekt när du konfigurerar Web SDK.
...
Använd inställningarna för **[!UICONTROL Device Lookup]** för att välja enhetsspecifik information som du vill samla in.
...
| [!UICONTROL Third Party ID Sync Container ID] | Det numeriska ID:t för behållaren som ska användas för synkronisering av ID från tredje part.<br><br>**Obs!** Datastream refererar till standardbehållar-ID för Audience Manager, som är 0. Om du har flera Audience Manager ID-synkroniseringsbehållar-ID:n kan du samarbeta med din Audience Manager-konsult för att identifiera och lösa synkroniseringskonfigurationen för ID. |
...
>[!NOTE]
>
>När du har sparat ändringar i en datastream-konfiguration kan det ta upp till 35 minuter innan ändringarna kan spridas i hela Edge Network. Under det här spridningsfönstret kan begäranden fortfarande hanteras med den tidigare konfigurationen.
...
### Adobe Advertising-inställningar {#advertising}

Den här tjänsten krävs för Adobe Advertising-integrering med Customer Journey Analytics.

### Adobe Analytics-inställningar {#analytics}

Den här tjänsten kontrollerar om och hur data skickas till Adobe Analytics.
...
| [!UICONTROL Visitor ID namespace] | (Valfritt) Det namnutrymme som du vill använda för Adobe Analytics [visitorID](https://experienceleague.adobe.com/docs/analytics/implementation/vars/config-vars/visitorid.html?lang=sv-SE). När du skickar en händelse med ett värde angivet för det här namnutrymmet används den automatiskt som `visitorID` i Analytics. <br> När fältet fylls i skickar datastream värdet `visitorID` till Adobe Analytics. Oavsett om besökar-ID:t ingår i `identityMap` genereras och inkluderas `ECID` fortfarande i utgående begäranden. Analyserna har stöd för att inkludera flera ID:n. ID:n utvärderas i den ordning som beskrivs på den här sidan: [Adobe Analytics ID-ordning för åtgärder](https://experienceleague.adobe.com/sv/docs/analytics/implementation/id/overview#adobe-analytics-identification-order-of-operations). |
...
Mer information finns i [Implementera Adobe Analytics med Edge Network](https://experienceleague.adobe.com/sv/docs/analytics/implementation/aep-edge/overview) i Analytics-implementeringsguiden.
...
![Adobe Audience Manager datastream-inställningar.](assets/configure/audience-manager-config.png)
...
| [!UICONTROL Offer Decisioning] | Aktiverar [Offer Decisioning](https://experienceleague.adobe.com/docs/journey-optimizer/using/offer-decisioning/get-started-decision/starting-offer-decisioning.html?lang=sv-SE) för Web SDK-implementeringar. |
| [!UICONTROL Edge Segmentation] | Aktiverar [kantsegmentering](../segmentation/methods/edge-segmentation.md) för det här datastream. När webb-SDK eller [Edge Network API](https://developer.adobe.com/data-collection-apis/docs/api/) skickar data via ett dataram med kantsegmentering aktiverat, skickas alla uppdaterade målgruppsmedlemskap för profilen i fråga tillbaka som svar.<br><br>Du kan använda det här alternativet i kombination med **Personalization-destinationer** för användning av samma sida och nästa sida genom [kantmål](../destinations/ui/activate-edge-personalization-destinations.md), [Offer Decisioning](https://experienceleague.adobe.com/sv/docs/journey-optimizer/using/decisioning/offer-decisioning/get-started-decision/starting-offer-decisioning), [Adobe Target](https://experienceleague.adobe.com/sv/docs/target) eller [Adobe Journey Optimizer](https://experienceleague.adobe.com/sv/docs/journey-optimizer/using/ajo-home) |
| [!UICONTROL Personalization Destinations] | Aktiverar [Anpassad Personalization](../destinations/catalog/personalization/custom-personalization.md) för det här dataströmmen. När webb-SDK eller [Edge Network API](https://developer.adobe.com/data-collection-apis/docs/api/) skickar data via ett datastödram med aktiverade anpassningsmål, skickas målgruppsmedlemskap och mappade profilattribut (endast för autentiserade [Edge Network API](https://developer.adobe.com/data-collection-apis/docs/api/) -begäranden) för profilen i fråga tillbaka som svar. |
...
I den här guiden beskrivs hur du hanterar datastölar i användargränssnittet för datainsamling. Mer information om hur du installerar och konfigurerar Web SDK efter att du har konfigurerat ett datastream finns i [Komma igång med taggtillägget Web SDK](../tags/extensions/client/web-sdk/getting-started.md).
