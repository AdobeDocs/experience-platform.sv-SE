...
Integreringsguiden för ETL innehåller allmänna steg för att skapa säkra anslutningar med höga prestanda för [!DNL Experience Platform] och inhämta data till [!DNL Platform].
...
Den här handboken innehåller även exempel på API-anrop som ska användas vid utformningen av en ETL-koppling, med länkar till dokumentation som anger var [!DNL Experience Platform] och hur dess API används i detalj.

Det finns en exempelintegrering på [!DNL GitHub] via [Referenskod för integrering av ETL-ekosystem](https://github.com/adobe/acp-data-services-etl-reference) under [!DNL Apache] Licens version 2.0.
...
Till att börja med loggar en ETL-användare in på [!DNL Experience Platform] användargränssnitt (UI) och skapar datauppsättningar för förtäring med hjälp av en standardanslutning eller push-tjänstanslutning.

I användargränssnittet skapar användaren utdata genom att välja ett dataschema. Vilket schema som väljs beror på vilken typ av data (post- eller tidsserie) som inhämtas till [!DNL Platform]. Genom att klicka på fliken Scheman i användargränssnittet kan användaren visa alla tillgängliga scheman, inklusive den beteendetyp som schemat stöder.

I ETL-verktyget kommer användaren att börja designa sina mappningstransformeringar efter att ha konfigurerat lämplig anslutning (med hjälp av sina autentiseringsuppgifter). ETL-verktyget antas redan ha [!DNL Experience Platform] installerade anslutningar (process definieras inte i den här integreringshandboken).

Mallar för ett ETL-exempelverktyg och arbetsflöde finns i [ETL-arbetsflöde](./workflow.md). ETL-verktygen kan ha olika format, men de flesta visar liknande funktioner.
...
[!DNL Catalog] svaren mäts automatiskt enligt konfigurerade gränser, men frågeparametern &quot;limit&quot; kan användas för att anpassa begränsningarna och begränsa antalet returnerade objekt. Den förkonfigurerade [!DNL Catalog] svarsgränserna är:
...
- Ogiltiga gränsparametrar (inklusive `limit=0`) möts med ett HTTP 400-fel som anger korrekta intervall.
- Om gränser eller förskjutningar skickas som frågeparametrar har de företräde framför dem som skickas som rubriker.

Frågeparametrar beskrivs mer ingående i [Katalogtjänst - översikt](../catalog/home.md).
...
Svaret innehåller tre (`limit=3`) som visar &quot;name&quot;, &quot;description&quot; och &quot;schemaRef&quot; enligt `properties` frågeparameter.
...
Det första &quot;schemaRef.id&quot;-värdet i det föregående svarsobjektet (`https://ns.adobe.com/{TENANT_ID}/schemas/274f17bc5807ff307a046bab1489fb18`) är en URI som pekar på ett specifikt XDM-schema i [!DNL Schema Registry]. Schemat kan hämtas genom att en sökbegäran (GET) görs till [!DNL Schema Registry] API.

>[!NOTE]
>
>Egenskapen &quot;schemaRef&quot; ersätter den nu inaktuella egenskapen &quot;schema&quot;. Om &quot;schemaRef&quot; saknas i datauppsättningen eller inte innehåller något värde, måste du kontrollera om det finns en &quot;schema&quot;-egenskap. Detta kan göras genom att ersätta &quot;schemaRef&quot; med &quot;schema&quot; i `properties` frågeparameter i föregående anrop. Mer information om egenskapen &quot;schema&quot; finns i [Egenskapen Schema för datauppsättningen](#dataset-schema-property-deprecated---eol-2019-05-30) följande avsnitt.
...
Begäran använder den URL som är kodad `id` URI för schemat (värdet för attributet &quot;schemaRef.id&quot;) och kräver en Accept-rubrik.
...
Svarsformatet beror på vilken typ av Acceptera-huvud som skickas i begäran. Uppslagsbegäranden kräver också en `version` tas med i sidhuvudet Godkänn. Följande tabell visar tillgängliga Accept headers för uppslag:
...
>[!NOTE]
>
>`application/vnd.adobe.xed-id+json` och `application/vnd.adobe.xed-full+json; version={major version}` är de vanligaste rubrikerna för Acceptera. `application/vnd.adobe.xed-id+json` är att föredra om du vill visa resurser i [!DNL Schema Registry] eftersom det bara returnerar &quot;title&quot;, &quot;id&quot; och &quot;version&quot;. `application/vnd.adobe.xed-full+json; version={major version}` är att föredra när du vill visa en viss resurs (med dess &quot;id&quot;) eftersom den returnerar alla fält (kapslade under &quot;properties&quot;) samt rubriker och beskrivningar.
...
Datauppsättningar kan innehålla en schemaegenskap som nu är föråldrad och som tillfälligt är tillgänglig för bakåtkompatibilitet. En listbegäran (GET) som liknar den som gjordes tidigare, där &quot;schema&quot; ersattes med &quot;schemaRef&quot; i `properties` frågeparameter, kan returnera följande:
...
Om schemaegenskapen för en datauppsättning fylls i signalerar detta att schemat är inaktuellt `/xdms` schema och, om det stöds, ETL-kopplingen ska använda värdet i egenskapen &quot;schema&quot; med `/xdms` slutpunkt (en inaktuell slutpunkt i [[!DNL Catalog API]](https://www.adobe.io/experience-platform-apis/references/catalog/)) för att hämta det äldre schemat.
...
>[!NOTE]
>
>En valfri frågeparameter, `expansion=xdm`, anger för API att expandera och infoga alla refererade scheman. Du kanske vill göra detta när du visar en lista över alla möjliga fält för användaren.
...
>[!NOTE]
>
>När schemafältet är tomt eller helt saknas, ska kopplingen läsa fältet &quot;schemaRef&quot; och använda [API för schemaregister](https://www.adobe.io/experience-platform-apis/references/schema-registry/) som i föregående steg till [visa ett datauppsättningsschema](#view-dataset-schema).
...
Ytterligare information, inklusive stegvisa anvisningar för att förhandsgranska data med hjälp av API:t för dataåtkomst, finns i [dataåtkomst, genomgång](../data-access/tutorials/dataset-data.md).

### Hämta datauppsättningsinformation med frågeparametern &quot;properties&quot;

Så som visas i stegen ovan till [visa en lista med datauppsättningar](#view-list-of-datasets)kan du begära &quot;filer&quot; med frågeparametern &quot;properties&quot;.

Du kan se [Katalogtjänst - översikt](../catalog/home.md) om du vill ha detaljerad information om hur du hämtar datauppsättningar och tillgängliga svarsfilter.
...
Svaret kommer att innehålla en datauppsättning (`limit=1`) som visar egenskapen &quot;files&quot;.
...
De datauppsättningsfil-ID som returnerades i det föregående svaret kan användas i en GET-begäran för att hämta ytterligare filinformation via [!DNL Data Access] API.

The [dataåtkomstöversikt](../data-access/home.md) innehåller information om hur du använder [!DNL Data Access] API.
...
| -------- | ----------- |
| `{DATASET_ID}` | The `id` värdet på den datauppsättning som du försöker få åtkomst till. |
...
När körningen startar kommer kopplingen (som definierats i källkomponenten) att läsa data från [!DNL Experience Platform] med [[!DNL Data Access API]](https://www.adobe.io/experience-platform-apis/references/data-access/). Omvandlingsprocessen läser data för ett visst tidsintervall. Internt kommer programmet att fråga batchar med källdataset. Vid sökning används en parametriserad (rullande för tidsseriedata eller inkrementella data) startdatum och listdatauppsättningsfiler för dessa grupper, och begäran om data för dessa datauppsättningsfiler börjar.

### Exempelomformningar

The [exempel på ETL-omformningar](./transformations.md) -dokumentet innehåller ett antal exempelomformningar, inklusive identitetshantering och datatypsmappningar. Använd dessa omformningar som referens.

### Läs data från [!DNL Experience Platform]

Använda [[!DNL Catalog API]](https://www.adobe.io/experience-platform-apis/references/catalog/)kan du hämta alla batchar mellan en angiven starttid och sluttid och sortera dem i den ordning som de skapades.
...
Information om filtergrupper finns i [Dataåtkomst, genomgång](../data-access/tutorials/dataset-data.md).

### Hämta filer från en grupp

När du har fått ID:t för gruppen du söker efter (`{BATCH_ID}`) går det att hämta en lista med filer som tillhör en specifik grupp via [[!DNL Data Access API]](https://www.adobe.io/experience-platform-apis/references/data-access/).  Information om hur du gör detta finns i [[!DNL Data Access] självstudiekurs](../data-access/tutorials/dataset-data.md).
...
Använda ett unikt ID för en fil (`{FILE_ID`), [[!DNL Data Access API]](https://www.adobe.io/experience-platform-apis/references/data-access/) kan användas för att komma åt specifik information om filen, inklusive namn, storlek i byte och en länk för att hämta den.
...
Svaret kan peka på en enskild fil eller en katalog. Mer information finns i [[!DNL Data Access] självstudiekurs](../data-access/tutorials/dataset-data.md).

### Åtkomst till filinnehåll

The [[!DNL Data Access API]](https://www.adobe.io/experience-platform-apis/references/data-access/) kan användas för att komma åt innehållet i en viss fil. För att hämta innehållet görs en GET-förfrågan med det värde som returnerats för `_links.self.href` vid åtkomst till en fil med fil-ID.
...
När data skrivs kan användarna välja att validera data enligt de valideringsregler som definieras i XDM-schemat. Mer information om schemavalidering finns i [ETL:s referenskod för integrering av ekosystem på [!DNL GitHub]](https://github.com/adobe/experience-platform-etl-reference/blob/fd08dd9f74ae45b849d5482f645f859f330c1951/README.md#validation).

Om du använder referensimplementeringen som finns på [[!DNL GitHub]](https://github.com/adobe/experience-platform-etl-reference/blob/fd08dd9f74ae45b849d5482f645f859f330c1951/README.md)kan du aktivera schemavalidering i den här implementeringen med hjälp av systemegenskapen `-DenableSchemaValidation=true`.

Validering kan utföras för logiska XDM-typer med attribut som `minLength` och `maxlength` för strängar, `minimum` och `maximum` för heltal med mera. The [Utvecklarhandbok för API för schematabell](../xdm/api/getting-started.md) innehåller en tabell som visar XDM-typer och egenskaper som kan användas för validering.

>[!NOTE]
>
>Minimi- och maximivärdena för olika `integer` är de MIN- och MAX-värden som typen kan hantera, men dessa värden kan begränsas ytterligare till de minimivärden och maximivärden som du väljer.
...
Information om hur du skapar en batch, inklusive exempelbegäranden och svar, finns i [Översikt över batchintag](../ingestion/batch-ingestion/overview.md).
...
När alla filer har överförts till gruppen kan gruppen signaleras för slutförande. Genom att göra det här [!DNL Catalog] DataSetFile-poster skapas för de slutförda filerna och kopplas till genereringsgruppen. The [!DNL Catalog] batchen markeras sedan som lyckad, vilket aktiverar efterföljande flöden för att importera tillgängliga data.
...
Innan du kör nya åtgärder i ETL-verktyget måste du se till att den senaste gruppen slutfördes korrekt. The [[!DNL Catalog Service API]](https://www.adobe.io/experience-platform-apis/references/catalog/) innehåller ett batchspecifikt alternativ som ger information om de relevanta batcharna.
...
En enskild batchstatus kan hämtas via [[!DNL Catalog Service API]](https://www.adobe.io/experience-platform-apis/references/catalog/) genom att skicka en GET-förfrågan med `{BATCH_ID}`. The `{BATCH_ID}` som användes skulle vara samma som det ID som returnerades när batchen skapades.
...
För att göra detta använder klientens dataadministratörer [!DNL Platform] Gränssnitt för att ta bort de batchar som innehåller skadade data. Därefter kommer ETL sannolikt att behöva köras igen, vilket innebär att korrekta data fylls i igen. Om själva källan innehåller skadade data måste datateknikern/administratören korrigera källbatcherna och importera data igen (antingen till Adobe Experience Platform eller via ETL-anslutningar).

Beroende på vilken typ av data som genereras blir det datateknikerns val att ta bort en enda batch eller alla batchar från vissa datauppsättningar. Data tas bort/arkiveras enligt [!DNL Experience Platform] riktlinjer.
...
Adobe Experience Platform identifierar för närvarande inte fördröjda data, så klientimplementeringar måste förlita sig på manuella konfigurationer för ETL och datauppsättning för att skapa en annan datauppsättning i [!DNL Platform] spegla källdatauppsättningen som kan användas för att behålla fördröjda data. I det här fallet liknar fördröjda data ögonblicksbildsdata. Vid varje körning av ETL-omvandlingen kommer källdata att förenas med fördröjda data och skickas för behandling.
...
| 2019-03-15 | Alla slutanvändaridentifierare mappas till egenskapen identityMap | &quot;identityMap&quot; är en inkapsling av alla unika identifierare för ett ämne, till exempel CRM-ID, ECID eller program-ID för lojalitet. Kartan används av [[!DNL Identity Service]](../identity-service/home.md) för att lösa alla kända och anonyma identiteter för ett motiv, som utgör ett enda identitetsdiagram för varje slutanvändare. |
| 2019-05-30 | EOL och Ta bort schemaegenskap från datauppsättningar | Datamängdens schemaegenskap tillhandahöll en referenslänk till schemat med hjälp av den borttagna `/xdms` slutpunkt i [!DNL Catalog] API. Detta har ersatts av en &quot;schemaRef&quot; som innehåller &quot;id&quot;, &quot;version&quot; och &quot;contentType&quot; för schemat enligt den nya [!DNL Schema Registry] API. |
