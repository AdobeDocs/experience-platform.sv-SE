...
Adobe rekommenderar att du lagrar medgivandedialogrutor separat från Web SDK-medgivandet. Web SDK erbjuder inte ett sätt att få samtycke. Om du vill vara säker på att användarinställningarna är synkroniserade med SDK kan du anropa kommandot `setConsent` vid varje sidinläsning. Web SDK gör bara ett serveranrop när medgivandet ändras.
...
När du har skickat användarinställningar till Web SDK med kommandot `setConsent`, kommer SDK att behålla användarinställningarna för en cookie. Nästa gång användaren läser in webbplatsen i webbläsaren hämtar och använder Web SDK dessa beständiga inställningar för att avgöra om händelser kan skickas till Adobe eller inte.

Du måste lagra användarinställningarna oberoende av varandra för att kunna visa dialogrutan för samtycke med de aktuella inställningarna. Det finns inget sätt att hämta användarinställningar från Web SDK. Om du vill vara säker på att användarinställningarna är synkroniserade med SDK kan du anropa kommandot `setConsent` vid varje sidinläsning. Web SDK gör bara ett serveranrop om inställningarna har ändrats.

## Synkronisera identiteter när du ställer in samtycke {#sync-identities}

När standardinställningen för samtycke (som anges med parametern [defaultConsent](configure/defaultconsent.md) ) är inställd på `pending` eller `out`, kan inställningen för `setConsent` vara den första begäran som går ut och upprättar identitet. På grund av detta kan det vara viktigt att synkronisera identiteter på den första begäran. Du kan lägga till identitetskartan till kommandot `setConsent` precis som med kommandot `sendEvent`. Se [Använda identityMap](../identity/overview.md#using-identitymap) för ett exempel på hur du inkluderar identitetskartan i ditt kommando.
