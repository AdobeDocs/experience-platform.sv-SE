...
* Publicera igen: En användare med lämplig `Publish` behörigheter kan nu publicera om ett tidigare publicerat bibliotek. Endast de fem sista biblioteken kan publiceras om.  Läs [Publicera om bibliotek](../ui/publishing/republish.md) för mer information.
...
Anteckningar: Nu kan du lägga till textanteckningar i `notable` resurser.  Dessa är tillägg, dataelement, regler, regelkomponenter och bibliotek.  Se [Anteckningar](../ui/managing-resources/notes.md) sida för mer information.

### Felkorrigeringar

* Hjälpdokumentsökningen har uppdaterats för att söka i den nya dokumentdatabasen
...
* Egenskapskopia: Nu kan du göra en kopia av en egenskap och allt i den. Välj en egenskap på listsidan Egenskaper och välj **[!UICONTROL Copy]** i verktygsfältet.
* Knappen Bygge arbetsbibliotek: För användare som använder arbetsbiblioteket finns nu en byggknapp. När du har angett arbetsbiblioteket och redigerat en resurs är standardåtgärden nu *Spara och lägg till* i stället för *Spara och bygg*. På så sätt kan användare redigera flera resurser och sedan bygga när du är klar.
* Visuella förbättringar av kodkontroller i kodredigeraren: Vi har gjort några visuella förbättringar för att mjuka upp utseendet på den kodvalidering som vi lanserade i augusti.
...
* Tokens i sökvägar: I nyligen skapade miljöer används nu mycket kortare ID:n för Company, Property, Extension och Environment i de sökvägar som refereras i inbäddningskoderna. Självvärdande användare stötte på begränsningar för sökvägslängd i Windows-system, så de har kortats av. Befintliga miljöer påverkas inte. Nya miljöer har mycket kortare sökvägar, inklusive de inbäddningskoder som refereras i installationsanvisningarna.
...
* Kodvalidering: Den anpassade kodredigeraren lints and minifiles your code behind the scenes och informerar dig om det finns problem. Vissa av dessa meddelanden är varningar, men vissa är fel som gör att ditt bygge misslyckas om koden ingår i ett bibliotek.
...
* Bibliotek som använder [!DNL Managed by Adobe] värd och en icke arkiverad miljö refererar nu till alla underfiler direkt med `https://` i stället för att ärva det underliggande sidprotokollet. På så sätt kan du använda taggar i inbäddade HTML-scenarier (särskilt för mobiler och digitalboxar) utan att vara värd.
* Du kan nu ta bort ditt arbetsbibliotek.
* Skärmen Redigera bibliotek har nu en knapp för att [!UICONTROL Remove All Resources].
* Klickmålets storlek har ökat för att ta bort enskilda resurser från ett bibliotek

### Felkorrigeringar

* Korrigerade ett fel i [!DNL Safari] som orsakade [!UICONTROL Add Resource] på sidan Biblioteksredigering som ska visas på fel plats.
...
* Felmeddelanden innehåller nu mycket mer information om felet som uppstod (för dem som vill titta). De är också ett enkelt sätt att logga biljetter och vissa nyckelidentifierare som kan hjälpa dig [!DNL Adobe] för att felsöka eventuella problem. Och de har flyttat till mitten längst ned på sidan för att anpassa sig till det senaste [!DNL Adobe] format.
* The [!UICONTROL Add Resource] knapparna på sidan Biblioteksredigering flyter nu när du rullar uppåt och nedåt så att de alltid visas.
...
* Korrigerade ett fel i Edge i Windows 10 som gjorde att biblioteksresurser inte kunde visas på sidan för biblioteksredigering.
...
The [!DNL Reactor] API som driver upp taggar når officiellt 1.0-status idag. Detta innebär att du måste behålla bakåtkompatibiliteten i den här större versionen (1.x-serien). För att komma igång med att arbeta med [!DNL Reactor] API, besök [Reaktors-API-guide](../api/overview.md).

### Uppdateringar

* Ett antal API-ändringar har gjorts för den här 1.0-versionen.
* `Adapter` har bytt namn till `Host`. Granskningshändelser innehåller följande objekt som är relaterade till den här ändringen.
   * För varje adapter visas en `Host.created` -händelse.
   ...
   * För varje adapter visas en `Adapter.deleted` -händelse.
...
* Jämför versioner - Du kan nu jämföra en version av en resurs med äldre versioner. Detta är tillgängligt för tillägg, dataelement och regler. Läs mer om den här nya funktionen [här](../ui/managing-resources/compare-resource-revisions.md).
...
* The `Enters Viewport` -händelsen kan nu konfigureras så att den utlöses varje gång elementet kommer in i visningsrutan i stället för bara första gången.
