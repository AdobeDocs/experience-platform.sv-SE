...
* **Alternativet** Behåll kohortAlternativet att behålla en kohort har lagts till i villkoret Prov. Detta medför att en användare hålls kvar i eller utanför exempelkohorten mellan sessionerna. Om till exempel kryssrutan &quot;beständig kohort&quot; är markerad och villkoret returnerar true första gången det körs för en viss besökare, returneras true för alla efterföljande körningar av villkoret för samma besökare. Om kryssrutan &quot;beständig kohort&quot; är markerad och villkoret returnerar false första gången det körs för en viss besökare, returneras false för alla efterföljande körningar av villkoret för samma besökare.
* **Felkorrigering** Ett problem har korrigerats där en regel som använder händelsen Sidnederst och en anpassad kod på en sida där taggar lästes in synkront men inte installerades korrekt (inget anrop till  `_satellite.pageBottom()`) skulle radera webbplatsinnehållet.
* **FelkorrigeringEtt** problem har korrigerats där Enters Viewport inte skulle fungera om taggbiblioteket lästes in asynkront och lästes in efter att webbläsarens DOMContentLoaded-händelse utlöstes.
...
* Ett problem har korrigerats där en regel som använder en biblioteksinläst händelse inte kunde utlösa en separat regel (försöker oftast utlösa en separat regel med `_satellite.track()`).

## 9 oktober 2018

### Uppdateringar

* Villkor och undantag har kombinerats i regelbyggaren eftersom ordningen är viktig.  Detta visar mer exakt hur de behandlas i systemet och under körning. Om du vill göra ett undantag markerar du knappen **[!UICONTROL Add]** under [!UICONTROL Conditions] och väljer **[!UICONTROL Exception]** från [!UICONTROL Logic Type]-väljaren.
* Om du använder värden som hanteras av [!DNL Adobe] rensas äldre versioner nu bort när nya byggen publiceras.
...
Om du skapar en mobil egenskap i användargränssnittet för datainsamling kan du:
...
* Hantera mobiltillägg:
   ...
   * Tillägg finns för andra Adobe-lösningar, inklusive [!DNL Adobe Analytics], [!DNL Adobe Target] och [!DNL Adobe Audience Manager].
...
* Cookie-namn för spårning har flyttats från egenskapsinställningarna till villkorsinställningarna [!DNL Adobe Analytics], [!DNL Google Universal Analytics] och Cookie-alternativ
...
Taggarna och [!DNL DTM]-biblioteken som använder [!DNL Akamai] är nu tillgängliga på CDN-kantnoder i Kina. Detta ger mycket snabbare biblioteksladdningshastigheter för slutanvändare i Kina.
...
Ändrat beteendet för knappen [!UICONTROL Add All Changes] på sidan [!UICONTROL Edit Library]. Tidigare ansågs en resurs bara vara&quot;ändrad&quot; förrän du lagt till den i ett bibliotek och sparat den. Nu betraktas en resurs som&quot;ändrad&quot; tills den har publicerats i produktionsmiljön.

#### Adobe Sekretesstillägg

Tillägget [!DNL Adobe Privacy] innehåller funktioner för att samla in och/eller ta bort användar-ID:n som tilldelats slutanvändare av [!DNL Adobe]-lösningar.
...
Dokumentation om taggar med öppen källkod finns på [Experience League](https://experienceleague.adobe.com/docs/launch/using/overview.html).
...
Cache-control-rubriker ställs nu in automatiskt för bibliotek som finns på [!DNL Akamai] (assets.adobedtm.com). Tidigare angav vi inte rubriker för cachekontroll för några filer på assets.adobedtm.com.
...
>[!IMPORTANT]
>
>Taggsutvecklare som inte har `-development` eller `-staging` i sin miljö måste skapa sina Development- och Staging-miljöer på nytt för att få huvudet 0 cache-control. Om du inte återskapar miljöerna får du samma 60-minuters cachekontroll som produktionsbiblioteken.
...
Händelser i regler kan nu tilldelas en order. När en händelse utlöses körs alla regler som använder den händelsen i den ordning som definieras. Lägre tal körs först (1 kommer före 10). Mer information finns i [Regelordning](../ui/managing-resources/rules.md#rule-ordering).
...
Adobe Experience Cloud-kunder får taggar som en inkluderad, värdeskapande funktion. Taggar är en helt ny produkt med en ny kodbas som är utformad för att ersätta den tidigare [!DNL Dynamic Tag Management (DTM)]-tjänsten. [!DNL DTM] kommer dock att ha fortsatt stöd under en överskådlig framtid. [!DNL Adobe] fortsätter att åtgärda eventuella viktiga fel och säkerställa enhetliga prestanda. För närvarande planeras inga större funktionsförbättringar för äldre [!DNL DTM].
...
Ett tillägg är ett kodpaket (JavaScript, HTML och CSS) som utökar användargränssnittet för datainsamling och klientfunktionerna. &#x200B;Bygg, hantera och uppdatera integreringarna med hjälp av ett praktiskt taget självbetjäningsgränssnitt. Du kan tänka dig en tagg som ett operativsystem, och tillägg är de program du använder för att utföra dina uppgifter.
...
* Taggar interagerar med &#x200B; för [!DNL Reactor]-API:er
