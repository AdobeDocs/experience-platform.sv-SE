...
* **Dataelement som har skapats som standard** ValueNew har inte längre tilldelats ett standardvärde automatiskt. Mer information finns i [Experience League Community](https://experienceleaguecommunities.adobe.com/t5/adobe-experience-platform-launch/coming-soon-data-element-quot-default-value-quot-changes/ba-p/374101)
...
* En tilläggsutvecklare kunde avbryta ett tilläggspaket som var under utveckling, vilket skapade ett problem där utvecklaren inte längre kunde POST eller PATCH av nya versioner av tilläggspaketet.  Detta har lösts genom att möjligheten att avbryta tilläggspaket under utveckling har tagits bort. Mer information finns i [Avsluta ett ExtensionPackage](https://developer.adobelaunch.com/api/reference/1.0/extension_packages/discontinue/).

## 7 juli 2020

### Funktioner

* **Regelkomponentssekvenser:** Kontrollera  **[!UICONTROL Run rule components in sequence]** i egenskapsinställningarna.  När det här alternativet är aktiverat läggs regelvillkor och åtgärder till i en bearbetningskö när regelhändelsen aktiveras.  Kön bearbetas FIFO-baserat med tidsgränser tillgängliga för enskilda komponenter.  På sidan [Regler](../ui/managing-resources/rules.md) finns mer information om tillgängliga inställningar och detaljer.
* **Löften i Core - Custom Code:** Du kan utföra asynkrona åtgärder i rutorna Core - Custom Code.  För att uppnå detta kan du returnera ett löfte från din anpassade JavaScript-kod eller använda de nya funktionerna `onCustomCodeSuccess()` och `onCustomCodeFailure()` i anpassad HTML-kod.  Mer information finns i [Core Extension reference](../extensions/web/core/overview.md) doc.
...
* Ett `no-cache`-huvud lades till i användargränssnittet. Tidigare var användarna tvungna att uppdatera för att få uppdateringar när JS-filer ändrades.
...
* **Avbryt tilläggspaket:** Tilläggspaketen kan nu avbrytas av tilläggsutvecklare. Ett avbrutet tilläggspaket är inte längre tillgängligt för nya installationer från taggkatalogen. Befintliga installationer fortsätter dock att fungera som förväntat. Mer information finns i [Avsluta ett ExtensionPackage](https://developer.adobelaunch.com/api/reference/1.0/extension_packages/discontinue/) i utvecklardokumenten.

## 9 maj 2020

### Felkorrigeringar

* När Safari &lt;= v12 användes, kunde användarna inte komma åt användargränssnittet för datainsamlingen på grund av ett `X-FRAME-OPTIONS`-huvud som angavs felaktigt.
...
* När en regelkomponent (händelse, villkor eller åtgärd) uppdateras uppdateras även attributet `updated_at` för regeln som den tillhör. Detta visar mer korrekt att beteendet för regeln har ändrats genom att en eller flera av dess komponenter har ändrats. Om du använder återanrop för regler kommer du att se en ökning av återanrop för regeln `updated_at`. Mer information finns i [https://developer.adobelaunch.com/api/reference/1.0/rule_components/](https://developer.adobelaunch.com/api/reference/1.0/rule_components/).
