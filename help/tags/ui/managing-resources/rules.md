...
1. Öppna fliken [!UICONTROL Rules] och välj sedan **[!UICONTROL Create New Rule]**.
   ...
   ![Fliken Regler markerar namnfältet.](../../images/launch-rule-builder.png)
...
1. Välj ikonen Åtgärder **[!UICONTROL Add]**, markera sedan tillägget och en av de tillgängliga åtgärdstyperna för tillägget, konfigurera inställningarna för åtgärden och välj sedan **[!UICONTROL Keep Changes]**.
...
>[!NOTE]
>
>I slutändan ligger ansvaret för att utföra åtgärder hos den utökade utvecklaren av den händelsetyp som du använder. Adobe tilläggsutvecklare ser till att deras tillägg fungerar som de ska. Adobe ger utvecklare av tillägg från tredje part vägledning så att de kan göra detta på rätt sätt, men kan inte garantera att dessa riktlinjer följs.
...
Inläsningsordningen för regler beror på om regelåtgärden har konfigurerats med JavaScript, HTML eller annan klientkod, och om reglerna använder en sidhändelse längst ned eller längst upp, eller en annan typ av händelse.
...
* **JavaScript:** JavaScript är inbäddat i huvudtaggbiblioteket. Det anpassade skriptet kapslas i en skripttagg och skrivs till dokumentet med `document.write`. Om regeln har flera anpassade skript skrivs de i ordning.
...
* **JavaScript:** JavaScript läses in från servern som vanlig text, omsluts av en script-tagg och läggs till i dokumentet med Postscribe. Om regeln har flera anpassade JavaScript-skript läses de in parallellt från servern, men körs i samma ordning som konfigurerats i regeln.
...
Körningsmiljöns beteende beror på om **[!UICONTROL Run rule components in sequence]** är aktiverat eller inte för egenskapen. Den här inställningen avgör om en regels komponenter kan utvärderas parallellt (asynkront) eller om de måste utvärderas sekventiellt.
