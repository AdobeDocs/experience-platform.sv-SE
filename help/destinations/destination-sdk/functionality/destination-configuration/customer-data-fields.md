...
|---------|----------|------|---|
| `name` | Sträng | Obligatoriskt | Ange ett namn för det anpassade fält som du introducerar. Det här namnet visas inte i plattformsgränssnittet, såvida inte fältet `title` är tomt eller saknas. |
...
| `title` | Sträng | Valfritt | Anger fältets namn, så som det visas av kunderna i plattformsgränssnittet. Om fältet är tomt eller saknas ärver gränssnittet fältnamnet från värdet `name`. |
...
I exemplet nedan definierar avsnittet `customerDataFields` två fält som användare måste ange i plattformsgränssnittet vid anslutning till målet:
...
När du skapar ett nytt mål lägger Destination SDK automatiskt till **[!UICONTROL Name]**- och **[!UICONTROL Description]**-fält på målanslutningsskärmen i plattformsgränssnittet. Som du kan se i exemplet ovan återges fälten **[!UICONTROL Name]** och **[!UICONTROL Description]** i användargränssnittet utan att inkluderas i konfigurationen för kunddatafält.
