...
[!DNL Adobe Experience Platform] Destination SDK är en serie konfigurations-API:er för att konfigurera målintegreringsmönster så att Experience Platform kan leverera målgrupps- och profildata till din slutpunkt eller lagringsplats, baserat på vilka data- och autentiseringsformat du väljer. Konfigurationerna lagras i Experience Platform och kan hämtas via API för ytterligare uppdateringar.

I Destination SDK-dokumentationen finns instruktioner om hur du kan använda [!DNL Adobe Experience Platform] Destination SDK för att konfigurera, testa och frisläppa en produkterad målintegrering med [!DNL Adobe Experience Platform] och få ditt mål att bli en del av den ständigt växande målkatalogen. Genom att använda Destination SDK kan du även skapa ett eget, anpassat privat mål för att exportera data som är anpassade efter dina behov.

![Skärmbild från Experience Platform-gränssnittet som visar målkatalogen.](assets/destinations-catalog-overview.png)
    ...
    <tr>
        ...
        <td>
            ...
            <ul>
                ...
                <li><a href="/help/destinations/destination-sdk/glossary.md">Ordlista för Destination SDK-termer</a></li>                
...
Som Destination SDK-partner kan du dra nytta av att lägga till ditt produkterade mål i [Experience Platform-katalogen](../catalog/overview.md):
...
2. Lägg in ett varumärkesprofilerat destinationskort i Experience Platform målkatalog för enklare kundkonfiguration och ökad medvetenhet.
3. Visa som en produkterad målintegration med [!DNL Adobe Experience Platform] &amp; Adobe [!DNL Real-Time Customer Data Platform].

Som Experience Platform-kund kan du också skapa en egen, anpassad destination som passar dina behov.
...
Via Destination SDK har [!DNL Adobe Experience Platform] stöd för realtidsintegrering (kallas även direktuppspelning) med mål som har en REST API-slutpunkt. Realtidsintegreringen med Experience Platform har funktioner som:
...
Med Destination SDK kan du också konfigurera integreringar för att regelbundet exportera filer till valfri lagringsplats. Den filbaserade integrationen med Experience Platform har funktioner som:

* Filexport i flera format som stöds (CSV, Parquet, JSON)
* Konfigurerbara filformateringsalternativ för att strukturera formatet för de exporterade filerna så att de uppfyller dina krav längre fram i kedjan.
...
Destination SDK åtkomst varierar beroende på din status som partner eller Experience Platform, [!DNL Real-Time CDP] kund. Se tabellen nedan för mer information.

| Typ av partner eller kund | Hitta Destination SDK |
|---------|----------|
...
| Experience Platform-kund i [Real-Time CDP Ultimate-paketet](https://helpx.adobe.com/se/legal/product-descriptions/real-time-customer-data-platform.html) | Som standard får du tillgång till Experience Platform sandlådor och Destination SDK, så att du kan skapa privata mål för din organisation. |
...
5. Om du är en ISV eller SI som skapar en [produktiserad integrering](./overview.md#productized-custom-integrations) [skickar du integreringen](guides/submit-destination.md) för Adobe granskning (standardsvarstiden är fem arbetsdagar).
6. Om du är en ISV- eller SI-leverantör och skapar en produkterad integrering använder du [självbetjäningsdokumentationsprocessen](docs-framework/documentation-instructions.md) för att skapa en produktdokumentationssida på Experience League för ditt mål.
7. När din integrering har godkänts av Adobe visas den i [Experience Platform-katalogen](../catalog/overview.md) för producerade integreringar.
