...
>[!NOTE]
>
>Om du vill lägga till `lastQualificationTime` i din filexport måste du för närvarande infoga värdet `xdm: segmentMembership.ups.seg_id.lastQualificationTime` manuellt i källfältet, vilket visas nedan. Du kan också redigera målfältet till `lastQualificationTime` eller något annat värde som du vill namnge den här kolumnen. Eftersom det här är en betafunktion kan syntaxen för värdet `xdm: segmentMembership.ups.seg_id.lastQualificationTime` komma att ändras i framtiden.
