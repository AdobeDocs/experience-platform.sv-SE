...
### Varför visas olika profilantal i Experience Platform-gränssnittet och i de exporterade CSV-filerna? {#profile-count-discrepancy}
...
### Varför ser jag låga matchningsfrekvenser när en uppdaterad målgrupp inaktiveras och återaktiveras till samma mål? {#low-match-rates-reactivation}
...
### När en målgrupp tas bort från ett mål, finns det någon signal som skickas till målet som anger att målgruppen tas bort? {#audience-removal-signal}
...
### Vad behöver jag göra innan jag kan aktivera målgrupper i [!DNL Facebook Custom Audiences]? {#facebook-activate-prerequisites}
...
* Affärskontot **[!DNL Adobe Experience Cloud]** måste läggas till som annonspartner i [!DNL Facebook Ad Account]. Använd `business ID=206617933627973`. Mer information finns i [Lägg till partner i din Business Manager](https://www.facebook.com/business/help/1717412048538897) i dokumentationen för Facebook.
  ...
  >[!IMPORTANT]
  >
  > När du konfigurerar behörigheterna för [!DNL Adobe Experience Cloud] måste du aktivera behörigheten **Hantera kampanjer**. Detta krävs för integreringen av [!DNL Adobe Experience Platform].
...
### Behöver jag lägga till några appar eller pixlar i mitt [!DNL Facebook]-annonserarkonto? {#facebook-pixels}

+++Svar
Nej. Eftersom detta inte är en pixelbaserad integrering behöver du inte lägga till några pixlar i annonskontot.
+++

### Hur lång tid tar det för Facebook att bearbeta information från [!DNL Adobe Experience Platform]? {#facebook-processing-time}
...
### Kan jag använda [!DNL Facebook Custom Audiences] för målgruppsanpassning i andra [!DNL Facebook]-appar, som [!DNL Instagram]? {#facebook-cross-app-targeting}
...
### Vad är skillnaden mellan anslutningen [!DNL Facebook Custom Audiences] och tillägget [!DNL Facebook Pixel]? {#facebook-custom-audiences-vs-pixel}
...
### Stöder integreringen av [!DNL Adobe Experience Platform] med [!DNL Facebook Custom Audiences] inaktivering av användare från en målgrupp när de inte längre är kvalificerade för det? {#facebook-disqualify-users}

+++Svar
Ja, integreringen stöder borttagning av användare från [!DNL Facebook Custom Audiences] när de inte längre är kvalificerade.
+++

### Hur ska jag hash-koda målgruppsdata innan jag skickar dem till [!DNL Facebook]? {#facebook-hashing}
...
### Vilken typ av identiteter kan jag aktivera i [!DNL Facebook Custom Audiences]? {#facebook-identities}
...
### Kan jag skapa flera Facebook-mål i Experience Platform-gränssnittet för separata Facebook-konton? {#facebook-multiple-destinations}

+++Svar
Ja. Ett Facebook-mål i Experience Platform är :1 till ett annonskonto i Facebook. Du kan skapa en separat Facebook-destination för varje Facebook-annonskonto i ditt företag. Följ självstudiekursen [för målanslutning](/help/destinations/ui/connect-destination.md) och anslut till ett separat Facebook-konto för varje nytt Facebook-mål i Experience Platform-gränssnittet. Det finns ingen gräns för hur många Facebook-annonskonton du kan ansluta till.
+++

## Google kundmatchning {#google-customer-match}

### När jag exporterar målgrupper till Google Customer Match, varför visas extra nummer i slutet av målgruppsnamnen i Google gränssnitt? {#google-customer-match-audience-name-numbers}
...
### Behöver jag lägga till några appar eller pixlar i mitt [!DNL LinkedIn]-annonserarkonto? {#linkedin-pixels}

+++Svar
Nej. Eftersom detta inte är en pixelbaserad integrering behöver du inte lägga till några pixlar i annonskontot.
+++

### Vad behöver jag göra innan jag kan aktivera målgrupper i [!DNL LinkedIn Matched Audiences]? {#linkedin-activate-prerequisites}
...
### Hur ska jag hash-koda målgruppsdata innan jag skickar dem till [!DNL LinkedIn]? {#linkedin-hashing}
...
### Vilken typ av identiteter kan jag aktivera i [!DNL LinkedIn]? {#linkedin-identities}
...
## Anpassa samma sida och nästa sida via [!DNL Adobe Target] och anpassade Personalization-mål {#same-next-page-personalization}

### Måste jag använda Experience Platform Web SDK för att skicka målgrupper och attribut till [!DNL Adobe Target]? {#target-web-sdk}
...
### Finns det någon gräns för hur många attribut jag kan skicka från kunddataplattformen i realtid till [!DNL Adobe Target] eller ett anpassat Personalization-mål? {#target-attributes-limit}

+++Svar
Ja, personalisering på samma sida och nästa sida stöder maximalt 30 attribut per sandlåda när målgrupper aktiveras för [!DNL Adobe Target] eller anpassade Personalization-mål. Mer information om aktiveringsskydd finns i [skyddsdokumentationen](guardrails.md#edge-destinations-activation).
+++

### Vilka typer av attribut stöds för aktivering (till exempel arrayer, kartor etc.)? {#target-supported-attribute-types}
...
### När jag har skapat en målgrupp i Experience Platform, hur lång tid tar det för den målgruppen att bli tillgänglig för användning av kantsegmentering? {#edge-segmentation-availability}

+++Svar
Målgruppsdefinitioner sprids till Edge Network på upp till en timme. Men om en målgrupp aktiveras inom den första timmen kan vissa besökare som är kvalificerade för målgruppen missas.
+++
...
### Vad händer om jag tar bort ett [!DNL Adobe Target]-mål? {#delete-target-destination}

+++Svar
När du tar bort ett mål tas alla målgrupper och attribut som är mappade under målet bort från [!DNL Adobe Target] och de tas också bort från Edge Network.
+++

### Fungerar integreringen med Edge Network API? {#edge-network-api-integration}

+++Svar
Ja, Edge Network API fungerar med Personalization-destinationen Custom. Eftersom profilattribut kan innehålla känsliga data måste du använda Edge Network API för datainsamling för att skydda dessa data på det anpassade Personalization-målet. Dessutom måste alla API-anrop göras i en [autentiserad kontext](https://developer.adobe.com/data-collection-apis/docs/getting-started/authentication/).
+++

### Jag kan bara ha en kopplingsregel som är aktiv i farten. Kan jag skapa målgrupper som använder en annan sammanfogningsprincip och skicka dem till [!DNL Adobe Target] som direktuppspelade målgrupper? {#edge-merge-policy}

+++Svar
Nej. Alla målgrupper som du vill aktivera för [!DNL Adobe Target] måste använda en [sammanfogningsprincip](../profile/merge-policies/ui-guide.md) som är aktiv.
+++

### Tillämpas etiketter och tvångsåtgärder (DULE) och godkännandeprinciper för dataanvändning? {#dule-consent-enforcement}
...
### Är [!DNL Adobe Target] och [!DNL Custom Personalization] mål [!DNL HIPAA]-kompatibla? {#hipaa-compliance}
