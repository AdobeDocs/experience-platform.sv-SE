...
Förutom vanliga SQL-frågor kan Adobe Experience Platform [!DNL Query Service] stöder skrivfrågor med [!DNL Experience Events]. Experience Events representeras av Experience Data Model (XDM) ExperienceEvent-klassen, som samlar in en oföränderlig och icke-aggregerad ögonblicksbild av systemet när en användare interagerar med en webbplats eller tjänst, och kan därför användas för tidsdomänanalys.

Mer information om XDM och [!DNL Experience Events] finns i [[!DNL XDM System] översikt](../../xdm/home.md). Genom att kombinera [!DNL Query Service] med [!DNL Experience Events]kan ni effektivt spåra beteendetrender bland era användare. Följande dokument innehåller exempel på frågor som innehåller [!DNL Experience Events].
...
Tidsstämpelkolumnen finns i [!DNL Experience Event] datauppsättningar är i UTC. I följande exempel används `from_utc_timestamp()` funktion för att omforma tidsstämpeln från UTC till EDT. Sedan används `date_format()` för att isolera datumet från resten av tidsstämpeln.
...
Mer information om exempelfrågor med hjälp av Adobe-definierade funktioner (ADF) finns i guiden Funktioner som definieras i Adobe. Allmänna riktlinjer för frågekörning finns i [guide för frågekörning i frågetjänsten](../best-practices/writing-queries.md).
