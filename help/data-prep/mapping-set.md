...
| `sourceType` | För varje mappning i listan anger dess `sourceType`-attribut vilken typ av källa som ska mappas. Kan vara en av `ATTRIBUTE`, `STATIC` eller `EXPRESSION`: <ul><li> `ATTRIBUTE` används för alla värden som finns i källsökvägen. </li><li>`STATIC` används för värden som matas in i målsökvägen. Detta värde förblir konstant och påverkas inte av källschemat.</li><li> `EXPRESSION` används för ett uttryck som kommer att tolkas under körning. En lista över tillgängliga uttryck finns i [guiden ](./functions.md) för mappningsfunktioner.</li> </ul> |
| `source` | För varje mappning i listan anger attributet `source` det fält som du vill mappa. Mer information om hur du konfigurerar källan finns i [källavsnittet](#sources). |
| `destination` | För varje mappning i listan anger attributet `destination` fältet eller sökvägen till fältet, där värdet som extraherats från fältet `source` kommer att placeras. Mer information om hur du konfigurerar dina mål finns i [målavsnittet](#destination). |
...
När du mappar ett fält i kolumndata, till exempel en CSV-fil, ska du använda källtypen `ATTRIBUTE`. Om fältet innehåller `.` i namnet använder du `\` för att undvika värdet. Ett exempel på den här mappningen finns nedan:
...
När du mappar ett fält i kapslade data, till exempel en JSON-fil, ska du använda källtypen `ATTRIBUTE`. Om fältet innehåller `.` i namnet använder du `\` för att undvika värdet. Ett exempel på den här mappningen finns nedan:
...
Med hjälp av källtypen `ATTRIBUTE` kan du även mappa en array direkt till en array eller ett objekt till ett objekt. Ett exempel på den här mappningen finns nedan:
...
När du vill mappa `source`-värdet till rotnivån för dina omformade data följer du exemplet nedan:
...
När du vill mappa `source`-värdet till ett kapslat fält i dina omformade data följer du exemplet nedan:
...
När du vill mappa `source`-värdet till ett specifikt index i en array i dina omformade data följer du exemplet nedan:
