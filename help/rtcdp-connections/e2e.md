...
Real-time Customer Data Platform (RTCDP) Connections samlar in och överför data till andra Adobe-produkter och tredjepartsmål. För att kunna skicka händelsedata från programmet till Adobe Experience Platform Edge Network är det viktigt att förstå dessa kärntekniker och hur de konfigureras för att leverera data till de mål du behöver, när du behöver det.

Den här guiden innehåller en självstudiekurs på hög nivå om hur du skickar en händelse via Edge Network med RTCDP-anslutningar. Självstudiekursen går igenom stegen för att installera och konfigurera taggtillägget Adobe Experience Platform Web SDK i användargränssnittet för datainsamling (tidigare Adobe Experience Platform Launch).
...
Processen med att konfigurera RTCDP-anslutningar för din webbplats kan sammanfattas på följande sätt:
