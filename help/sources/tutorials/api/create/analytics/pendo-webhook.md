---
title: Skapa en Source-anslutning och ett dataflöde för Pendo med API:t för Flow Service
...
I följande självstudiekurs får du hjälp med att skapa en källanslutning och ett dataflöde för att skicka [[!DNL Pendo]](https://Pendo.com/) händelsedata till Adobe Experience Platform med [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
## Anslut [!DNL Pendo] till Experience Platform med API:t [!DNL Flow Service] {#connect-platform-to-flow-api}

Följande beskriver de steg du måste utföra för att skapa en källanslutning och ett dataflöde för att kunna överföra [!DNL Pendo]-händelsedata till Experience Platform.
...
| `data.format` | Formatet på de [!DNL Pendo]-data som du vill importera. För närvarande är det enda dataformat som stöds `json`. |
...
| `data.format` | Formatet på de [!DNL Pendo]-data som du vill importera. |
...
Det sista steget mot att överföra data från [!DNL Pendo] till Experience Platform är att skapa ett dataflöde. Nu har du förberett följande obligatoriska värden:
...
Ett lyckat svar returnerar information om ditt dataflöde, inklusive din slutpunkts-URL, markerad som `inletUrl`. Gå till sidan [Konfigurera webkrok](../../../ui/create/analytics/pendo-webhook.md#get-streaming-endpoint-url) för att hämta det värde som krävs.
