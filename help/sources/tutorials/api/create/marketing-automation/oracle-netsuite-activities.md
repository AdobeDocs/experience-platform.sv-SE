...
description: Lär dig hur du skapar en källanslutning och ett dataflöde för att överföra data för NetSuite-händelser för Oracle till Experience Platform med hjälp av API:t för Flow Service.
hide: true
hidefromtoc: true
badge: Beta
...
Läs följande självstudiekurs för att lära dig hur du kan hämta händelsedata från [!DNL Oracle NetSuite Activities] till Adobe Experience Platform med [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
I följande avsnitt finns ytterligare information som du behöver känna till för att kunna ansluta till [!DNL Oracle NetSuite Activities] med [!DNL Flow Service] API.
...
## Anslut [!DNL Oracle NetSuite Activities] till plattform med [!DNL Flow Service] API

Följ guiden nedan för att lära dig hur du autentiserar [!DNL Oracle NetSuite Activities] , skapa en källanslutning och skapa ett dataflöde för att skicka händelsedata till Experience Platform.
...
Om du vill skapa ett basanslutnings-ID skickar du en POST till `/connections` slutpunkt när du ger [!DNL Oracle NetSuite Activities] autentiseringsuppgifter som en del av begärandetexten.
...
| `{SOURCE_PARAMS}` | Definierar parametrar för källfilen som du vill hämta till plattformen. Hämta den godkända formattypen för `{SOURCE_PARAMS}`måste du koda hela strängen i base64. <br> Värdet är tomt för [!DNL Oracle NetSuite Activities].</li></ul> |
...
| `data.format` | Formatet på [!DNL Oracle NetSuite Activities] data som du vill importera. För närvarande är det enda dataformatet som stöds `json`. |
...
| `data.format` | Formatet på [!DNL Oracle NetSuite Activities] data som du vill importera. |
...
Det sista steget mot att hämta in data från [!DNL Oracle NetSuite Activities] till Platform är att skapa ett dataflöde. Nu har du förberett följande obligatoriska värden:
