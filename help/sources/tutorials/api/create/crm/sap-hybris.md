---
title: Skapa en källanslutning och ett dataflöde för SAP Hybris med API:t för Flow Service
description: Lär dig hur du skapar en källanslutning och ett dataflöde för att överföra SAP Hybris-data till Experience Platform med API:t för Flow Service.
badge: Beta
hide: true
hidefromtoc: true
...
# Skapa en källanslutning och ett dataflöde för [!DNL SAP Hybris] med API:t för Flow Service
...
I följande självstudiekurs får du hjälp med att skapa en [!DNL SAP Hybris] källanslutning och ett dataflöde som kan ge [[!DNL SAP] Prenumerationsfakturering](https://www.sap.com/products/financial-management/subscription-billing.html) kontakter och kunddata till Adobe Experience Platform med [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
I följande avsnitt finns ytterligare information som du behöver känna till för att kunna ansluta till [!DNL SAP Hybris] med [!DNL Flow Service] API.
...
Mer information om dessa uppgifter finns i [[!DNL SAP Hybris] dokumentation](https://help.sap.com/docs/CLOUD_TO_CASH_OD/987aec876092428f88162e438acf80d6/c5fcaf96daff4c7a8520188e4d8a1843.html).

## Anslut [!DNL SAP Hybris] till plattform med [!DNL Flow Service] API

Följande beskriver de steg du måste utföra för att autentisera dina [!DNL SAP Hybris] källa, skapa en källanslutning och skapa ett dataflöde för att överföra dina konton och kontaktdata till Experience Platform.

### Skapa en basanslutning {#base-connection}

En basanslutning bevarar information mellan källan och plattformen, inklusive källans autentiseringsuppgifter, anslutningsstatus och ditt unika basanslutnings-ID. Med det grundläggande anslutnings-ID:t kan du utforska och navigera bland filer inifrån källan och identifiera de specifika objekt som du vill importera, inklusive information om deras datatyper och format.

Om du vill skapa ett basanslutnings-ID skickar du en POST till `/connections` slutpunkt när du ger [!DNL SAP Hybris] autentiseringsuppgifter som en del av begärandetexten.
...
Följande begäran skapar en basanslutning för [!DNL SAP Hybris]:
...
| `{SOURCE_PARAMS}` | Definierar parametrar för källfilen som du vill hämta till plattformen. Hämta den godkända formattypen för `{SOURCE_PARAMS}`måste du koda hela strängen i base64. <br> [!DNL SAP Hybris] har stöd för flera API:er. Beroende på vilken objekttyp du använder kan du skicka något av följande: <ul><li>`customers`</li><li>`contacts`</li></ul> |

The [!DNL SAP Hybris] har stöd för flera API:er. Beroende på vilken objekttyp du utnyttjar den begäran som ska skickas anges nedan:

>[!NOTE]
>
>Vissa svarsposter har trunkerats för att ge en bättre presentation.
...
För [!DNL SAP Hybris] Kunder-API:t har värdet för `{SOURCE_PARAMS}` skickas som `{"object_type":"customers"}`. När den kodas i base64 är den lika med `eyJvYmplY3RfdHlwZSI6ImN1c3RvbWVycyJ9` enligt nedan.
...
För [!DNL SAP Hybris] Kontaktpersonens API-värde för `{SOURCE_PARAMS}` skickas som `{"object_type":"contacts"}`. När den kodas i base64 är den lika med `eyJvYmplY3RfdHlwZSI6ImNvbnRhY3RzIn0=` enligt nedan.
...
Följande begäran skapar en källanslutning för [!DNL SAP Hybris] kunddata:
...
| `baseConnectionId` | Basanslutnings-ID för [!DNL SAP Hybris]. Detta ID genererades i ett tidigare steg. |
| `connectionSpec.id` | Det ID för anslutningsspecifikation som motsvarar källan. |
| `data.format` | Formatet på [!DNL SAP Hybris] data som du vill importera. För närvarande är det enda dataformatet som stöds `json`. |
| `object_type` | [!DNL SAP Hybris] har stöd för flera API:er. För kund-API:t är `object_type` parametern ska anges till `customers`. |
...
Följande begäran skapar en källanslutning för [!DNL SAP Hybris] kontaktdata:
...
| `connectionSpec.id` | Det ID för anslutningsspecifikation som motsvarar källan. |
| `data.format` | Formatet på [!DNL SAP Hybris] data som du vill importera. För närvarande är det enda dataformatet som stöds `json`. |
| `object_type` | [!DNL SAP Hybris] har stöd för flera API:er. För kontakt-API:t `object_type` parametern ska anges till `contacts`. |
...
Följande begäran skapar en målanslutning för [!DNL SAP Hybris]:
...
| `data.format` | Formatet på [!DNL SAP Hybris] data som du vill importera. |
...
I följande begäran skapas en mappning för [!DNL SAP Hybris] Kundens API-data
...
I följande begäran skapas en mappning för [!DNL SAP Hybris] API-data för kontakter
...
Det sista steget mot att hämta in data från [!DNL SAP Hybris] till Platform är att skapa ett dataflöde. Nu har du förberett följande obligatoriska värden:
