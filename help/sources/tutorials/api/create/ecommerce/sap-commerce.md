...
# Skapa en källanslutning och ett dataflöde för [!DNL SAP Commerce] med API:t för Flow Service
...
I följande självstudiekurs får du hjälp med att skapa en [!DNL SAP Commerce]-källanslutning och ett dataflöde för att skicka [[!DNL SAP] prenumerationsfakturering](https://www.sap.com/products/financial-management/subscription-billing.html)-kontakter och kunddata till Adobe Experience Platform med [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
| --- | --- |
| `clientId` | Värdet `clientId` från tjänstnyckeln. |
| `clientSecret` | Värdet `clientSecret` från tjänstnyckeln. |
| `tokenEndpoint` | Värdet `url` från tjänstnyckeln liknar värdet `https://subscriptionbilling.authentication.eu10.hana.ondemand.com`. |
| `region` | Datacentrets plats. Regionen finns i `url` och har ett värde som liknar `eu10` eller `us10`. Om `url` till exempel är `https://subscriptionbilling.authentication.eu10.hana.ondemand.com` behöver du `eu10`. |
...
Följande begäran skapar en basanslutning för [!DNL SAP Commerce]:
...
| `auth.params.tokenEndpoint` | Värdet `url` från tjänstnyckeln liknar värdet `https://subscriptionbilling.authentication.eu10.hana.ondemand.com`. |
...
För [!DNL SAP Commerce]-kund-API:t skickas värdet för `{SOURCE_PARAMS}` som `{"object_type":"customers"}`. När den kodas i base64 är den lika med `eyJvYmplY3RfdHlwZSI6ImN1c3RvbWVycyJ9` enligt nedan.
...
För [!DNL SAP Commerce] kontakter-API:t skickas värdet för `{SOURCE_PARAMS}` som `{"object_type":"contacts"}`. När den kodas i base64 är den lika med `eyJvYmplY3RfdHlwZSI6ImNvbnRhY3RzIn0=` enligt nedan.
...
Följande begäran skapar en källanslutning för [!DNL SAP Commerce] kunddata:
...
| `data.format` | Formatet på de [!DNL SAP Commerce]-data som du vill importera. För närvarande är det enda dataformat som stöds `json`. |
| `object_type` | [!DNL SAP Commerce] har stöd för flera API:er. För kund-API:t ska parametern `object_type` anges till `customers`. |
...
Följande begäran skapar en källanslutning för [!DNL SAP Commerce] kontaktdata:
...
| `connectionSpec.id` | Anslutningsspecifikations-ID som motsvarar källan. |
| `data.format` | Formatet på de [!DNL SAP Commerce]-data som du vill importera. För närvarande är det enda dataformat som stöds `json`. |
| `object_type` | [!DNL SAP Commerce] har stöd för flera API:er. För kontakt-API:t ska parametern `object_type` anges till `contacts`. |
| `path` | Detta har samma värde som du väljer för *`object_type`*. |
...
Följande begäran skapar en målanslutning för [!DNL SAP Commerce]:
...
| `data.format` | Formatet på de [!DNL SAP Commerce]-data som du vill importera. |
...
I följande begäran skapas en mappning för [!DNL SAP Commerce]-kunders API-data
...
Följande begäran skapar en mappning för [!DNL SAP Commerce] kontakter-API-data
