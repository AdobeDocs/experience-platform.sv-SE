...
| `1003-404` | Resursen hittades inte | Den begärda resursen hittades inte. Kontrollera den angivna begäran och försök igen. |
...
| `1012-504` | Tidsgräns för gateway | En gatewaytimeout har inträffat. Försök igen och kontakta kundsupport om problemet kvarstår. |
...
| `1014-400` | Ogiltig begäran Ogiltigt argument | Det gick inte att bearbeta begäran. {detailedMessage} |

## Ramfel

| Felkod | Titel | Detaljerat meddelande |
| --- | --- | --- |
| `1100-400` | Felaktig begäran | Det gick inte att bearbeta begäran. {detailedMessage} |
| `1101-500` | Internt fel | Ett internt fel har inträffat. Försök igen och kontakta kundsupport om problemet kvarstår. |
| `1102-404` | Resursen hittades inte | Den begärda resursen hittades inte. {detailedMessage} |
...
| `1202-400` | Felaktig begäran | PublicKeyId saknas i flödets transformationer={transformations}. Ange publicKeyId i begäran och försök igen. |
| `1203-400` | Felaktig begäran | Krypteringsnyckeln finns inte mot keyID={keyID} i flödets transformationer={transformations}. Kontrollera ditt nyckel-ID och försök igen. |
...
| Felkod | Titel | Detaljerat meddelande |
| --- | --- | --- |
| `1300-400` | Felaktig begäran | Korrigeringsanslutningsbegäran stöds inte för {connectorType}-anslutningen. Kontrollera den angivna begäran och försök igen. |
| `1301-400` | Felaktig begäran | Angiven authSpecType-param: {authSpecType} stöds inte. Ange en giltig autentiseringstyp och försök igen. |
...
| `1303-400` | Felaktig begäran | Det gick inte att koda URL:en med angivna auth-parametrar eftersom URL-kodning inte stöds för {value}. Kontrollera dina auth-parametrar och försök igen. |
...
| `1308-400` | Felaktig begäran | Det gick inte att bearbeta begäran. Felmeddelande: {msftErrorMessage} |
...
| 1312-400 | Felaktig begäran | Det angivna parametervärdet {paramValue} är inte en giltig inmatning för egenskap={requestParam}. Ange ett giltigt parametervärde och försök igen. |
...
| `1315-400` | Felaktig begäran | Den angivna sökvägen {path} ska peka på en matris med poster. Kontrollera att den angivna sökvägen pekar på arkivmatrisen med poster och försök igen. |
...
| `1320-400` | Felaktig begäran | {idType} får inte vara null eller tom. Ange giltig {idType} och försök igen. |
...
| `1323-400` | Felaktig begäran | {entity} får inte vara null eller tom i källanslutningen: {sourceConnection}. Ange giltig {entity} och försök igen. |
...
| `1325-400` | Felaktig begäran | Parametern dataSetId får inte vara null eller tom i målanslutningen: {TargetConnection}. Ange en giltig dataSetId-param och försök igen. |
...
| `1328-400` | Felaktig begäran | Mappningsomformningen kan inte vara null när {param} extraheras. Ange en giltig mappningsomvandling och försök igen. |
| `1329-400` | Felaktig begäran | Param: {param} får inte vara null eller tom i den angivna begäran. Ange giltig {param} och försök igen. |
| `1330-400` | Felaktig begäran | Inga kolumner hittades för tabellen {tableName}. Ange ett giltigt tabellnamn och försök igen. |
| `1331-400` | Felaktig begäran | Kolumnavgränsaren kan inte vara mer än ett tecken i SourceConnection: {sourceConnection}. Ange en giltig kolumnavgränsare och försök igen. |
...
| `1334-400` | Felaktig begäran | Frågeparam: {param} får inte vara tom. Ange giltig {param} och försök igen. |
...
| `1337-400` | Felaktig begäran | {QueryParam} får inte vara tom för objectType={objectType}. Ange giltig {QueryParam} och försök igen. |
...
| `1365-400` | Felaktig begäran | Sökvägen {path} som angavs i OP är ogiltig. Ange en giltig sökväg {path} och försök igen. |
...
| `1379-400` | Felaktig begäran | Det uppstod ett fel när indata {value} för filtret vid källan verifierades. Kolumnen DataType vid källan är {columnDataType} och värdet DataType [String] matchar inte. Ange en giltig {value} och försök igen. |
...
| `1382-400` | Felaktig begäran | Deltakolumnen ska matcha värdet som finns i flödets Kopiera-omformningar. Ange en giltig delta-kolumn och försök igen. |
...
| `1384-400` | Felaktig begäran | Parametrarna={params} som har angetts för att skapa flödeskörning är ogiltiga eller tomma. Ange giltiga parametrar och försök igen. |
...
| `1387-400` | Felaktig begäran | flowRunId={flowRunId} är i ett ogiltigt tillstånd={state} för återutlösande. Försök igen om en stund och kontakta kundsupport om problemet kvarstår. |
| `1388-400` | Felaktig begäran | Flödet={flow} är i tillståndet={state} och kan inte återaktiveras. Flödet måste vara i aktiverat läge för att kunna aktiveras igen. |
...
| `1394-400` | Felaktig begäran | Det lägsta värdet för parametern {parameter} är ett. Ange en giltig {parameter} och försök igen. |
...
| Felkod | Titel | Detaljerat meddelande |
| --- | --- | --- |
| `1600-400` | Felaktig begäran | Det gick inte att bearbeta begäran. {detailedMessage} |
| `1601-500` | Internt fel | Ett internt fel har inträffat. Försök igen och kontakta kundsupport om problemet kvarstår. |
| `1602-404` | Resursen hittades inte | Den begärda resursen hittades inte. {detailedMessage} |
...
| `1604-504` | Tidsgräns för gateway | En gatewaytimeout har inträffat. Försök igen. Kontakta kundsupport om problemet kvarstår. |
...
| `1702-400` | Felaktig begäran | SAS-typen {sasType} som har angetts för landningszonen är inte tillåten. Ange en giltig SAS och försök igen. |
...
| `1710-400` | Felaktig begäran | Den angivna landningszonen av typen {landingZoneUseCase} stöds inte. Ange en giltig typ av landningszon och försök igen. |
...
| `1714-400` | Felaktig begäran | clientId={clientId} kan inte utföra den här åtgärden. Kontrollera din begäran med behörigheter och försök igen. |
