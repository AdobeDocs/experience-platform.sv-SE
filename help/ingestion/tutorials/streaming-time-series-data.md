...
>[!NOTE] Den här datauppsättningen aktiveras för kundprofil **och** identitet **i** realtid genom att rätt taggar anges.
...
Med datauppsättningen och direktuppspelningsanslutningen på plats kan du importera XDM-formaterade JSON-poster för att importera tidsseriedata inom plattformen.
...
>[!NOTE] Du måste skapa din egen `xdmEntity._id` och `xdmEntity.timestamp`. Ett bra sätt att generera ett ID är att använda ett UUID. Dessutom kräver följande API-anrop **inga** autentiseringshuvuden.
