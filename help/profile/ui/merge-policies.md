...
Information om alla kopplingsregler som är tillgängliga för din organisation finns på landningssidan, inklusive principnamn, standardkopplingsregler och schema.
...
När profilposter hämtas till Experience Platform hämtas en systemtidsstämpel vid tidpunkten för inmatningen och läggs till i posten. När **[!UICONTROL Timestamp ordered]** är markerat som&quot;[!UICONTROL Attribute merge]&quot;-typ för en sammanfogningsprincip sammanfogas profiler baserat på systemets tidsstämpel. Sammanfogningen görs med andra ord baserat på den tidsstämpel som användes när posten hämtades till Platform.
...
Om du vill använda en anpassad tidsstämpel måste du lägga till&quot;[!UICONTROL External Source System Audit Details Mixin]&quot; i profilschemat. När du har lagt till den anpassade tidsstämpeln kan du fylla i den med hjälp av `lastUpdatedDate` fältet.
...
När **[!UICONTROL Edit merge policy]** skärmen visas kan du ändra namn, schema, ID-sammanfogningstyp och attributsammanfogningstyp, samt välja om den här principen ska vara organisationens standardprincip för sammanfogning eller inte.
...
Det här meddelandet innehåller en lista över dataanvändningsprinciper som har överträtts och gör att du kan visa information om överträdelsen genom att välja en princip i listan. När du har valt en obehörig princip anger fliken orsaken till överträdelsen och de aktiveringar som påverkas], där de ger mer information om hur dataanvändningsprincipen har överträtts. **[!UICONTROL Data lineage]**
