...
* För återkommande besökare hämtas [!DNL ECID] från cookien [`kndctr_<orgId>_identity`](https://experienceleague.adobe.com/sv/docs/core-services/interface/data-collection/cookies/web-sdk) och läggs till i nyttolasten för begäran av Edge Network.
...
Datainsamling från tredje part innebär att data skickas direkt till Edge Network-domänen `adobedc.net`.
...
När du använder datainsamling från tredje part begränsar vissa annonsblockerare dessutom trafiken till Adobe datainsamlingsslutpunkter helt och hållet.
...
Använd [Dataprep för datainsamling](/help/datastreams/data-prep.md) för att mappa [!DNL ECID] till ett [!DNL XDM]-fält. Det här är det rekommenderade sättet att komma åt [!DNL ECID].
...
![Datastream mapping, bild](/help/tags/extensions/client/web-sdk/assets/access-ecid-data-prep.png)
...
Om du vill hämta CORE-ID:t för en användare kan du använda kommandot [`getIdentity()`](/help/collection/js/commands/getidentity.md) enligt nedan.
...
`identityMap` fält uppdateras med kommandot `sendEvent`.
...
Om du för närvarande har [vidarebefordring av händelser](/help/tags/ui/event-forwarding/overview.md) aktiverat och använder `appmeasurement.js` och `visitor.js` kan du behålla funktionen för vidarebefordran av händelser aktiverad och detta orsakar inga problem. På baksidan hämtar Adobe alla AAM-segment och lägger till dem i anropet till Analytics. Om anropet till Analytics innehåller dessa segment kommer Analytics inte att ringa Audience Manager för att vidarebefordra data, så det finns ingen dubbel datainsamling. Du behöver heller inte använda platstips när du använder Web SDK eftersom samma segmenteringsslutpunkter anropas i serverdelen.
