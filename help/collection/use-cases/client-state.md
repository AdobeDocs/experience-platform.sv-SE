...
Själva Edge Network är statslös (den har inte en egen session). Det finns dock vissa användningsfall som kräver att klientsidans tillstånd är beständigt, till exempel:
...
Om du vill ha stöd för flera taggar (d.v.s. flera SDK-instanser i samma egenskap, som eventuellt refererar till olika organisationer), kommer alla tillståndsposter automatiskt att få prefixet `kndctr_` och URL-säkra organisations-ID:n.

När klienten SDK tar emot en `state:store`-referens i svaret måste den göra följande:
...
När du arbetar med webbläsarklienter kan Edge Network automatiskt behålla posterna som cookies i webbläsaren. Detta ger stöd för genomskinlig lagring eftersom webbläsare som standard respekterar protokollet för tillståndshantering.
...
Eftersom posterna alltid är bundna till ett specifikt omfång (enhet/program) enligt definition, kommer endast den delmängd som är kompatibel med den aktuella begärandekontexten att skrivas av Edge Network. Oskrivna poster
...
Flaggan `Secure` finns inte i metadatarepresentationen för cookies. Endast attributet `SameSite` inkluderas. I det här fallet är det klientens ansvar att ange flaggan `Secure` korrekt när attributet `SameSite` finns. Cookies med `SameSite=None` måste även ange attributet `Secure` eftersom de kräver en säker kontext (HTTPS).
